...
function log(data) {
    document.querySelector('#debug').innerHTML = JSON.stringify(data, null, 2)
}

ipcRenderer
    .on('params', (event, arg) => {
        console.log('params', event, arg)
        params = arg
        initProcessing()
    })
...
let fpsCnt = 0
    , procFpsCnt = 0 // processed screenshots per second
function fpsTick() {
    fpsCnt++
...
var fpsInterval = setInterval(() => {
    ...
    document.getElementById('fps').innerHTML = fpsCnt + ' FPS ' + procFpsCnt + ' SPS'
    fpsCnt = 0
    procFpsCnt = 0
}, 1000)

function initProcessing() {
    console.log('Waiting for 00:00:00...')
    ...
    classifierContext = document.querySelector('#classifier').getContext('2d')
}
...
function initCapture(video, slices) {
    ...
    function draw() {
        ...
        if (
            ...
            if (result.time === '00:00:00' && !startCollection) {
                console.log('Started collection of data.')
            ...
            log('bad frame')
            if (startCollection) {
                console.log('bad frame', JSON.stringify(result))
            }
        }
